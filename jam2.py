...
def ___menu___():
    ...
    if ___menu___ in ['1','01']:
        ...
        print("\n%s[%s1%s]%s Lihat RIAZ Ok.txt"%(B,P,B,P))
        print("%s[%s2%s]%s Lihat RIAZ Cp.txt"%(B,P,B,P))
        ...
        if ___hasilz___ in ['1','01']:
            try:
                ...
                exit("%s[%s!%s]%s RIAZ Ok.txt Tidak Ada"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Ok.txt')
            print("\n%s[%s*%s]%s Total RIAZ Ok.txt :%s %s"%(B,P,B,P,H,len(open('Results/Ok.txt','r').readlines())))
        elif ___hasilz___ in ['2','02']:
            try:
                ___cp___ = open('Results/Cp.txt','r').read()
            except (IOError):
                exit("%s[%s!%s]%s RIAZ Cp.txt Tidak Ada"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Cp.txt')
            print("\n%s[%s*%s]%s Total RIAZ Cp.txt :%s %s"%(B,P,B,P,H,len(open('Results/Cp.txt','r').readlines())))
        ...
        os.system("xdg-open https://wa.me/+923188214452?text=Hallo%20Bang%20Rozhak")
        exit()
